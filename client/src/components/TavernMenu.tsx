            ...
            {activeCategory === 'profile' ? (
              ...
              <div className="achievements-container p-4">
                ...
                {user ? (
                  ...
                  <div className="text-center py-8 text-[#E8D6B3] font-['VT323'] text-xl">
                    Please sign in first to earn badges.
                  </div>
                )}
              </div>
            ) : activeCategory === 'lore' ? (
                ...
                {filteredItems.length > 0 ? (
                  <div className="grid gap-4 grid-cols-1">
                    {filteredItems.map(item => (
                      <div 
                        key={item.id}
                        className="menu-item flex p-3 bg-[#2C1810] border border-[#8B4513] rounded-sm cursor-pointer hover:bg-[#3C281A]"
                        onClick={() => orderMenuItem(item.id)}
                      >
                        <div className="item-icon w-16 h-16 bg-[#2C1810] flex items-center justify-center rounded-sm">
                          <MenuItemIcon itemName={item.icon} className="w-12 h-12" />
                        </div>
                        <div className="item-details flex-1">
                          <h3 className="font-['VT323'] text-[#FFD700] text-lg">{item.name}</h3>
                          <p className="text-[#E8D6B3] text-sm opacity-80 font-['VT323']">{item.description}</p>
